...
// #include "stm32f10x.h" // 注释掉或移除标准库头文件
#include "stm32f1xx_hal.h"        // 添加 HAL 库主头文件 (通常包含 hal_gpio.h 等)
#include "OLED.h"
#include <string.h>
#include <math.h>
#include <stdio.h>
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	// GPIO_WriteBit(OLED_GPIO_PORT, OLED_SCL_PIN, (BitAction)BitValue); // 注释掉标准库调用
	HAL_GPIO_WritePin(OLED_GPIO_PORT, OLED_SCL_PIN, (GPIO_PinState)BitValue); // 使用 HAL 库调用
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	// GPIO_WriteBit(OLED_GPIO_PORT, OLED_SDA_PIN, (BitAction)BitValue); // 注释掉标准库调用
	HAL_GPIO_WritePin(OLED_GPIO_PORT, OLED_SDA_PIN, (GPIO_PinState)BitValue); // 使用 HAL 库调用
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
	// RCC_APB2PeriphClockCmd(RCC_APB2Periph_OLED_GPIO_PORT, ENABLE); // 注释掉标准库时钟使能

	// GPIO_InitTypeDef GPIO_InitStructure; // 注释掉标准库结构体定义
	// GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD; // 注释掉标准库模式设置
	// GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; // 注释掉标准库速度设置
	// GPIO_InitStructure.GPIO_Pin = OLED_SCL_PIN; // 注释掉标准库引脚设置
	// GPIO_Init(OLED_GPIO_PORT, &GPIO_InitStructure); // 注释掉标准库初始化调用
	// GPIO_InitStructure.GPIO_Pin = OLED_SDA_PIN; // 注释掉标准库引脚设置
	// GPIO_Init(OLED_GPIO_PORT, &GPIO_InitStructure); // 注释掉标准库初始化调用
    ...
    GPIO_InitStructure.Pin = OLED_SCL_PIN | OLED_SDA_PIN; // 配置 PB8 和 PB9
    ...
    HAL_GPIO_Init(OLED_GPIO_PORT, &GPIO_InitStructure);        // 初始化 OLED_GPIO_PORT 的指定引脚
